    ...
    <table class="min-w-full mt-2 border">
        <thead>
            <tr class="bg-gray-100">
                ...
                <th class="px-4 py-2 text-left">Predicted Sales</th>
        ...
        <tbody>
            ...
            <tr class="border-t">
                ...
                <td class="px-4 py-2">KSh {{ number_format($forecast->predicted_sales, 2) }}</td>
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const forecastLabels = {!! json_encode($forecastData->pluck('forecast_date')->map(fn($d) => \Carbon\Carbon::parse($d)->format('M d'))) !!};
    const forecastValues = {!! json_encode($forecastData->pluck('predicted_sales')) !!};

    const historicalLabels = {!! json_encode($historicalSales->pluck('date')->map(fn($d) => \Carbon\Carbon::parse($d)->format('M d'))) !!};
    const historicalValues = {!! json_encode($historicalSales->pluck('quantity')) !!};
